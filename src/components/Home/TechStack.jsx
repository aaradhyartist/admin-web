      ...
      <div className="w-full px-6 lg:px-16">
        ...
        <div className="flex flex-col lg:flex-row items-start justify-between gap-8 mb-20">
          <div className="max-w-2xl">
            ...
            <h2 className="text-4xl md:text-5xl font-black text-slate-900 leading-tight tracking-tighter uppercase">
             ...
             <span className="text-[#DC2626]"> Business.</span>
            </h2>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-3 gap-6">
          {categories.map((item, index) => (
            <div
              key={index}
              className="flex items-center gap-6 p-6 bg-slate-50 rounded-[2rem] border border-slate-300 hover:border-[#DC2626]/30 hover:bg-white hover:shadow-xl hover:shadow-slate-200/40 transition-all duration-500 group cursor-default"
            >
              {/* LEFT: Icon Container - Fixed size so it doesn't shrink */}
              <div className="flex-shrink-0 w-20 h-20 rounded-2xl bg-white flex items-center justify-center text-slate-400 group-hover:bg-[#DC2626] group-hover:text-white transition-all duration-500 shadow-sm">
                {React.cloneElement(item.icon, { size: 32, strokeWidth: 2.5 })}
              </div>

              {/* RIGHT: Content - Aligned to the left */}
              <div className="flex flex-col justify-center text-left">
                <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight mb-1 group-hover:text-[#DC2626] transition-colors">
                  {item.name}
                </h3>
