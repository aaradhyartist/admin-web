      ...
      <div className="mx-auto px-6 lg:px-16">
        ...
        <div className="flex flex-wrap gap-3 mb-12">
          {categories.map((cat) => (
            <button
              ...
              key={cat}
              className={`px-5 py-2 rounded-full text-xs font-black uppercase tracking-widest border border-slate-200 text-slate-600 ${selectedCategory === cat ? "bg-[#DC2626] text-white " : ""} hover:cursor-pointer hover:bg-slate-200 hover:text-black transition-all`}
            >
              {cat}
            </button>
          ))}
            ...
            <div key={index} className="group relative bg-slate-50 rounded-[1.5rem] overflow-hidden border border-slate-100 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-slate-200">
              ...
              <div className="p-8">
                <div className="flex items-center gap-3 mb-4">
                  <span className="p-2 bg-white rounded-lg text-[#DC2626] shadow-sm">
                    {project.icon}
                  </span>
                ...
                <h4 className="text-xl font-black text-slate-900 mb-3 group-hover:text-[#DC2626] transition-colors">
                  {project.title}
                </h4>
        ...
        <div className="mt-12 p-10 bg-slate-900 rounded-[2rem] flex flex-col lg:flex-row items-center justify-between gap-8">
          ...
          <button
            ...
            className="hover:cursor-pointer px-8 py-4 bg-white text-slate-900 rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-[#DC2626] hover:text-white transition-all whitespace-nowrap"
          >
